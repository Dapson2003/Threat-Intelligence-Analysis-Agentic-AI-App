...
async def send_to_next_agent_json(data_str):
    try:
        ...
        result_raw = result_wrapper.get("result", "")
        if result_wrapper.get("status") != "success" or not isinstance(result_raw, str):
            print("⚠️ Invalid result format or not successful.")
            return None
        ...
        print("✅ Cleaned and published parsed result.")
        return result_json

    except Exception as e:
        print(f"❌ Exception during JSON handling: {e}")
        return None
