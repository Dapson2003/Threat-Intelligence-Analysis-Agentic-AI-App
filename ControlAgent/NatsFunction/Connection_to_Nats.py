...
async def CloseService():
    ...
    """
    try:
        await stop_nats_subscriber(cfg.INPUT_SUBJECT1)
        await stop_nats_subscriber(cfg.INPUT_SUBJECT2)
        await stop_nats_subscriber(cfg.INPUT_SUBJECT3)
        if nc.is_connected:
            await nc.drain()
...
async def OpenService():
    ...
    try:
        ...
        if nc.is_connected:
            print("⚠️ Previous NATS connection still alive — draining it before reconnecting.")
            await nc.drain()
            await nc.close()

        await start_nats_subscriber_with_js(
            subject=cfg.INPUT_SUBJECT1,
            durable_name=cfg.DURABLE_NAME1,
            queue_name=cfg.QUEUE_NAME,
        )
        await start_nats_subscriber_with_js(
            subject=cfg.INPUT_SUBJECT2,
            durable_name=cfg.DURABLE_NAME2,
            queue_name=cfg.QUEUE_NAME,
        )
        await start_nats_subscriber_with_js(
            subject=cfg.INPUT_SUBJECT3,
            durable_name=cfg.DURABLE_NAME3,
            queue_name=cfg.QUEUE_NAME,
        )
    except Exception as e:
        if "consumer name already in use" in str(e).lower() or "already bound" in str(e).lower():
            print(f"ℹ️ Subscriber already exists on subject '{cfg.INPUT_SUBJECT1}' with durable '{cfg.DURABLE_NAME}' — continuing.")
        else:
